...
class OrderResource extends Resource
{
    ...
    public static function infolist(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Section::make('Support')->schema([TextEntry::make('support_open')->label('Open tickets')->state(fn(Order $record)=>$record->supportTickets()->whereNotIn('status',['resolved','closed'])->count()),TextEntry::make('support_total')->label('Total tickets')->state(fn(Order $record)=>$record->supportTickets()->count()),TextEntry::make('support_latest')->label('Latest ticket')->state(fn(Order $record)=>optional($record->supportTickets()->first(),fn($t)=>$t->ticket_number.' · '.str($t->status)->replace('_',' ')->title().' · '.str($t->priority)->title().' · '.($t->assignee?->name??'Unassigned').' · '.($t->last_message_at?->diffForHumans()??'No messages'))??'No support tickets')->url(function(Order $record){$ticket=$record->supportTickets()->first();return $ticket?SupportTicketResource::getUrl('view',['record'=>$ticket]):null;})])->columns(3),

...
class WorkerDocumentResource extends Resource {
 ...
 public static function table(Table $t):Table{return $t->columns([TextColumn::make('application.email')->label('Applicant'),TextColumn::make('document_type')->badge()->formatStateUsing(fn($s)=>str($s)->replace('_',' ')->title()),TextColumn::make('status')->badge()->formatStateUsing(fn($s)=>str($s)->replace('_',' ')->title()),IconColumn::make('required')->boolean(),IconColumn::make('manually_verified')->boolean(),TextColumn::make('file_path')->placeholder('No uploaded file')])->filters([SelectFilter::make('status')->options(['pending'=>'Pending','submitted'=>'Submitted','approved'=>'Approved','rejected'=>'Rejected','expired'=>'Expired'])])->recordActions([EditAction::make()]);}

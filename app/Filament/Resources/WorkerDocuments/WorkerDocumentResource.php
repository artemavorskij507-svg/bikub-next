...
class WorkerDocumentResource extends Resource {
 ...
 public static function table(Table $t):Table{return $t->columns([TextColumn::make('application.email')->label('Applicant'),TextColumn::make('document_type')->badge()->formatStateUsing(fn($s)=>str($s)->replace('_',' ')->title()),TextColumn::make('status')->badge()->formatStateUsing(fn($s)=>str($s)->replace('_',' ')->title()),TextColumn::make('compliance_status')->badge()->placeholder('Not evaluated')->formatStateUsing(fn($s)=>str($s)->replace('_',' ')->title()),TextColumn::make('risk_level')->badge()->placeholder('Not evaluated')->formatStateUsing(fn($s)=>str($s)->title()),IconColumn::make('required')->boolean(),IconColumn::make('has_media')->label('Evidence')->state(fn(WorkerDocument $record)=>$record->hasMedia('worker_documents'))->boolean(),IconColumn::make('manually_verified')->boolean(),TextColumn::make('expires_at')->date(),TextColumn::make('support_tickets_count')->counts('supportTickets')->label('Support'),TextColumn::make('updated_at')->since()])->filters([SelectFilter::make('status')->options(['pending'=>'Pending','submitted'=>'Submitted','approved'=>'Approved','rejected'=>'Rejected','expired'=>'Expired'])])->recordActions([

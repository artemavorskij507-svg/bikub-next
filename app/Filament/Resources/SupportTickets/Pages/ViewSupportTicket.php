...
class ViewSupportTicket extends ViewRecord {
 ...
 protected function getHeaderActions():array{return [
  ...
  Action::make('customer_message')->schema([Textarea::make('body')->required()->helperText('Visible later when the customer support portal is enabled.')])->action(fn(array $data)=>app(SupportTicketService::class)->addMessage($this->record,['body'=>$data['body'],'message_type'=>'public_reply','visibility'=>'customer_visible'],auth()->user())),
  Action::make('worker_message')->schema([Textarea::make('body')->required()->helperText('Visible later when the worker support portal is enabled.')])->action(fn(array $data)=>app(SupportTicketService::class)->addMessage($this->record,['body'=>$data['body'],'message_type'=>'public_reply','visibility'=>'worker_visible'],auth()->user())),

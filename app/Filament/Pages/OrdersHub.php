...
class OrdersHub extends AdminOsModulePage
{
    ...
    public function calculateQuote(int $id): void { $this->runFinanceAction(fn()=>app(QuoteCalculationService::class)->calculateForOrder(Order::findOrFail($id),auth()->user()),'Real quote calculated.'); }
    public function createDraftInvoice(int $id): void { $this->runFinanceAction(fn()=>app(BillingDocumentService::class)->createDraftInvoiceForOrder(Order::findOrFail($id),auth()->user()),'Draft invoice created.'); }
    public function getViewData(): array
    {
        ...
        return ['metrics'=>['active'=>Order::whereIn('status',$active)->count(),'waiting'=>Order::whereIn('status',['submitted','accepted'])->count(),'unassigned'=>Order::whereIn('status',['submitted','accepted'])->whereDoesntHave('dispatchAssignments',fn($q)=>$q->whereIn('status',['assigned','accepted']))->count(),'in_progress'=>Order::where('status','in_progress')->count(),'completed_today'=>Order::whereDate('completed_at',today())->count(),'support'=>Order::whereHas('supportTickets',fn($q)=>$q->whereNotIn('status',['resolved','closed']))->count(),'missing_owner'=>Order::whereNull('customer_id')->count(),'payment_not_ready'=>Order::whereIn('payment_status',['pending','failed'])->count(),'with_gps'=>WorkerLocationPing::whereNotNull('order_id')->distinct('order_id')->count('order_id'),'without_gps'=>Order::whereIn('status',$active)->whereDoesntHave('workerLocationPings')->count()], 'queue'=>$this->queue()->get(),'selectedOrder'=>$selected,'assignment'=>$selected?->activeDispatchAssignment(),'quote'=>$selected?->latestPriceQuote(),'latestInvoice'=>$selected?->billingDocuments()->latest()->first(),'latestTicket'=>$selected?->supportTickets->first(),'openTickets'=>$selected?->supportTickets->whereNotIn('status',['resolved','closed'])??collect(),'completion'=>$selected?app(OrderCompletionService::class)->getCompletionState($selected):null,'settlement'=>$selected?app(WorkerSettlementService::class)->getSettlementReadiness($selected):null,'blockers'=>$selected?app(OrderHealthService::class)->evaluate($selected):[],'timeline'=>$this->timeline($selected),'paymentProviderEnabled'=>(bool)($operations?->payment_provider_enabled??false),'customerTrackingEnabled'=>(bool)($operations?->customer_tracking_enabled??false)];
    }
    private function base(): Builder { return Order::with(['scenario','customer','priceQuotes','workerSettlementEntries','dispatchAssignments.assignedUser.workerProfile','dispatchAssignments.assignedUser.workerAvailability','dispatchEvents','supportTickets.assignee','workerLocationPings','completionProofs.events','events']); }
    ...
    private function runFinanceAction(callable $action,string $success):void { abort_unless(auth()->user()?->can('admin.finance.manage'),403); try{$action();Notification::make()->title($success)->success()->send();}catch(ValidationException $e){Notification::make()->title(collect($e->errors())->flatten()->first())->warning()->send();} }
}

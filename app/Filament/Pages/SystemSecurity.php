...
class SystemSecurity extends AdminOsModulePage
{
    ...
    public function getOperationalModules(): array
    {
        ...
        return [
            ...
            $this->status('Support Center', \App\Models\SupportTicket::count().' ticket(s)', 'Tables/resource active; '.\App\Models\SupportTicket::whereNotIn('status',['resolved','closed'])->count().' open ticket(s). Order, Dispatch Center and WorkerDocument integrations active. Account portal, worker portal, database notifications, email, SMS and realtime are deferred.', $this->routeAvailable('filament.admin.resources.support-tickets.index') ? 'configured' : 'blocked', $this->routeAvailable('filament.admin.resources.support-tickets.index') ? route('filament.admin.resources.support-tickets.index') : ''),

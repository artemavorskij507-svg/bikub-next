...
class SystemSecurity extends AdminOsModulePage
{
    ...
    public function getOperationalModules(): array
    {
        ...
        return [
            ...
            $this->status('Support Center', \App\Models\SupportTicket::count().' ticket(s)', 'Admin dashboard/resource, order, Dispatch Center, WorkerDocument, account portal, worker portal and protected attachments are configured. Internal notes remain Admin OS-only. Database notifications are blocked because the notifications table is absent; email, SMS and realtime are deferred.', $this->routeAvailable('filament.admin.pages.support') && $this->routeAvailable('account.support.index') && $this->routeAvailable('worker.support.index') && $this->routeAvailable('admin.support.attachments.download') ? 'configured' : 'blocked', $this->routeAvailable('filament.admin.pages.support') ? route('filament.admin.pages.support') : ''),

...
class WorkerSettlementRuleReviewService {
    public function requestReview(WorkerSettlementRule $rule,string $type,User $actor,?string $note):WorkerSettlementRuleReview{
        throw_unless($actor->can('admin.settlement_reviews.request'),ValidationException::withMessages(['actor'=>'Settlement review request permission is required.']));
    ...
    public function approveReview(WorkerSettlementRuleReview $review,User $actor,string $decisionNote,string $evidenceSummary):WorkerSettlementRuleReview{
        $this->assertApprover($review,$actor);throw_if(blank($decisionNote),ValidationException::withMessages(['decision_note'=>'Decision note is required.']));throw_if(blank($evidenceSummary),ValidationException::withMessages(['evidence_summary'=>'Evidence summary is required.']));
        if($review->rule->created_by_id===$actor->id){throw_unless($actor->can('admin.settlement_reviews.override_separation'),ValidationException::withMessages(['separation'=>'Rule creator cannot approve this review without an audited separation override.']));$this->event($review,$actor,'separation_override_used','Creator/reviewer separation override used: '.$decisionNote);}
    ...
    public function rejectReview(WorkerSettlementRuleReview $review,User $actor,string $reason):WorkerSettlementRuleReview{throw_unless($actor->can('admin.settlement_reviews.reject')||$actor->can($this->permission($review->review_type)),ValidationException::withMessages(['actor'=>'Review rejection permission is required.']));return $this->transition($review,$actor,'rejected','rejected',$reason);}
    public function cancelReview(WorkerSettlementRuleReview $review,User $actor,string $reason):WorkerSettlementRuleReview{throw_unless($actor->can('admin.settlement_reviews.cancel'),ValidationException::withMessages(['actor'=>'Review cancellation permission is required.']));return $this->transition($review,$actor,'cancelled','cancelled',$reason);}
    ...
    private function assertApprover(WorkerSettlementRuleReview $review,User $actor):void{throw_unless($actor->can($this->permission($review->review_type)),ValidationException::withMessages(['actor'=>ucfirst($review->review_type).' reviewer permission is required.']));}
    private function permission(string $type):string{return 'admin.settlement_reviews.'.$type.'.approve';}
}

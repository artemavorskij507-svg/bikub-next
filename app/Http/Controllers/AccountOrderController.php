...
class AccountOrderController extends Controller {
 ...
 public function show(Request $request,Order $order,CustomerOwnershipService $ownership){abort_unless($ownership->canViewOrder($request->user(),$order),403);return view('account.orders.show',['order'=>$order->load(['scenario','priceQuotes','dispatchAssignments','supportTickets'=>fn($query)=>$query->where('customer_id',$request->user()->id),'billingDocuments'=>fn($query)=>$query->whereIn('status',['issued','paid','refunded'])])]);}
}

...
@if($selectedOrder)<section class="fc-panel"><div class="fc-panel-head"><div><span>WORKER SETTLEMENT LEDGER</span><h2>{{ $settlement['entry']?->entry_number ?? 'Not calculated' }}</h2></div><b>{{ $settlement['entry']?->status ?? 'blocked' }}</b></div><p>Rule: {{ $settlement['rule']?->name ?? 'No active approved rule' }}</p><p>Payout provider: {{ $payoutReadiness['provider']['configured'] ? 'Configured' : 'Disabled' }}</p><div class="fc-actions"><button wire:click="calculateSettlement({{$selectedOrder->id}})">Calculate settlement</button><a href="{{$settlementRulesUrl}}">Settlement rules</a><button disabled title="{{$payoutReadiness['next_action']}}">Prepare payout unavailable</button><button disabled title="Payout provider is not configured.">Mark paid unavailable</button></div>@if($settlement['entry'])<p>Gross: {{number_format((float)$settlement['entry']->gross_amount,2)}} {{$settlement['entry']->currency}}</p><p>Worker amount: {{$settlement['entry']->worker_amount===null?'Blocked until an approved settlement rule is configured.':number_format((float)$settlement['entry']->worker_amount,2).' '.$settlement['entry']->currency}}</p><p>Platform fee: {{$settlement['entry']->platform_fee_amount===null?'Not calculated':number_format((float)$settlement['entry']->platform_fee_amount,2).' '.$settlement['entry']->currency}}</p>@endif @foreach($payoutReadiness['blockers'] as $reason)<article class="fc-blocker"><strong>Payout blocked</strong><p>{{$reason}}</p></article>@endforeach</section>@endif
</main>
<style>

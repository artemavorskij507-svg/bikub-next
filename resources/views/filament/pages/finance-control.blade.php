...
@if($selectedOrder)<section class="fc-panel"><div class="fc-panel-head"><div><span>WORKER SETTLEMENT LEDGER</span><h2>{{ $settlement['entry']?->entry_number ?? 'Not calculated' }}</h2></div><b>{{ $settlement['entry']?->status ?? 'blocked' }}</b></div><div class="fc-actions"><button wire:click="calculateSettlement({{$selectedOrder->id}})">Calculate settlement</button><button disabled title="Settlement is not ready.">Approve unavailable</button><button disabled title="Payout provider/manual payout workflow is not configured.">Mark paid unavailable</button></div>@if($settlement['entry'])<p>Gross: {{number_format((float)$settlement['entry']->gross_amount,2)}} {{$settlement['entry']->currency}}</p><p>Worker amount: {{$settlement['entry']->worker_amount===null?'Blocked until settlement rule is configured.':number_format((float)$settlement['entry']->worker_amount,2).' '.$settlement['entry']->currency}}</p>@endif @foreach($settlement['blockers'] as $reason)<article class="fc-blocker"><strong>Payout blocked</strong><p>{{$reason}}</p></article>@endforeach</section>@endif
</main>
<style>

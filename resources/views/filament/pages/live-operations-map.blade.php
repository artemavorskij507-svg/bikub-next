        ...
        <main class="mx-map-panel live-processing-glow" aria-label="Live operations map workspace">
            ...
            <div class="mx-stage" wire:ignore>
                ...
                <div id="mx-context" class="mx-context" role="menu" aria-label="Map actions" tabindex="-1">
                    ...
                    <button type="button" role="menuitem" data-action="zone" data-zone="service_area">Create service zone here</button>
                    <button type="button" role="menuitem" data-action="zone" data-zone="no_go_area">Create no-go zone here</button>
                    <button type="button" role="menuitem" data-action="zone" data-zone="priority_area">Create priority zone here</button>
                    <button type="button" role="menuitem" data-action="zone" data-zone="support_incident">Create support incident here</button>
                    <button type="button" role="menuitem" data-action="dispatch-note" @disabled(!$assignment?->order) title="{{ $assignment?->order ? 'Create a real dispatch event for the selected active order.' : 'Select an active order first.' }}">Add dispatch note at location</button>
                    <button type="button" role="menuitem" data-action="support" @disabled(!$assignment?->order) title="{{ $assignment?->order ? 'Create a real support ticket linked to the selected active order.' : 'Select an order first or create a support incident zone.' }}">Create support ticket at location</button>
                    <button type="button" role="menuitem" disabled data-reason="Nearby search requires order/worker coordinates. No real coordinates available yet." title="Nearby search requires order/worker coordinates. No real coordinates available yet.">Search nearby orders/workers<small>No real coordinates available yet</small></button>
...
const menu=document.getElementById('mx-context'),status=document.getElementById('mx-context-status'),fallback=document.getElementById('mx-coordinate-fallback');const closeMenu=()=>menu.classList.remove('open'),closeEditors=()=>document.querySelectorAll('.mx-editor').forEach(e=>e.classList.remove('open')),showStatus=(m,bad=false)=>{status.textContent=m;status.style.color=bad?'#f5bd54':'#55e6aa'},component=()=>{const root=container.closest('[wire\:id]');return root&&window.Livewire?.find(root.getAttribute('wire:id'))},call=async(method,...args)=>{const c=component();if(!c){showStatus('LiveOps action connection unavailable. Refresh the page.',true);return false}menu.setAttribute('aria-busy','true');showStatus('Opening action?');try{await c.call(method,...args);return true}catch(e){showStatus(e?.message||'Action could not be opened.',true);return false}finally{menu.removeAttribute('aria-busy')}};
L.DomEvent.disableClickPropagation(menu);L.DomEvent.disableScrollPropagation(menu);
map.on('contextmenu',e=>{ctx=e.latlng;const value=`${ctx.lat.toFixed(6)}, ${ctx.lng.toFixed(6)}`;fallback.value=value;showStatus('Choose an operational action.');const p=map.latLngToContainerPoint(ctx),stage=document.querySelector('.mx-stage');menu.classList.add('open');menu.style.left=Math.max(8,Math.min(p.x,stage.clientWidth-menu.offsetWidth-8))+'px';menu.style.top=Math.max(8,Math.min(p.y,stage.clientHeight-menu.offsetHeight-8))+'px';document.getElementById('mx-coordinates').textContent=value;document.getElementById('mx-external-map').href=`https://www.openstreetmap.org/?mlat=${ctx.lat}&mlon=${ctx.lng}#map=16/${ctx.lat}/${ctx.lng}`;menu.querySelector('[role=menuitem]:not(:disabled)')?.focus({preventScroll:true})});map.on('click',closeMenu);document.addEventListener('pointerdown',e=>{if(menu.classList.contains('open')&&!menu.contains(e.target)&&!container.contains(e.target))closeMenu()});document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeMenu();closeEditors()}if(menu.classList.contains('open')&&['ArrowDown','ArrowUp','Home','End'].includes(e.key)){e.preventDefault();const items=[...menu.querySelectorAll('[role=menuitem]:not(:disabled)')],i=items.indexOf(document.activeElement);const n=e.key==='Home'?0:e.key==='End'?items.length-1:e.key==='ArrowDown'?(i+1)%items.length:(i-1+items.length)%items.length;items[n]?.focus()}});
menu.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const item=e.target.closest('[role=menuitem]');if(!item)return;if(item.disabled){showStatus(item.dataset.reason||item.title||'This action is unavailable.',true);return}if(item.tagName==='A'){window.open(item.href,'_blank','noopener');showStatus('External OpenStreetMap opened.');closeMenu();return}if(!ctx)return showStatus('Right-click the map first.',true);const a=item.dataset.action;if(a==='zone'&&await call('openContextEditor','zone',ctx.lat,ctx.lng,item.dataset.zone))closeMenu();if(a==='dispatch-note'&&await call('openContextEditor','dispatch',ctx.lat,ctx.lng,null))closeMenu();if(a==='support'&&await call('openContextEditor','support',ctx.lat,ctx.lng,null))closeMenu();if(a==='copy'){const value=`${ctx.lat.toFixed(6)}, ${ctx.lng.toFixed(6)}`;try{await navigator.clipboard.writeText(value);showStatus('Coordinates copied.')}catch(err){fallback.value=value;fallback.focus();fallback.select();showStatus('Clipboard unavailable. Coordinates selected for manual copy.',true)}}});const actionComplete=e=>{closeEditors();closeMenu();showStatus(e.detail?.message||'Action completed.');refresh()};window.addEventListener('liveops-action-completed',actionComplete);document.getElementById('mx-refresh').addEventListener('click',refresh);refresh();const timer=setInterval(refresh,Math.max(10,d.refresh_seconds||12)*1000);const destroy=()=>{if(destroyed)return;destroyed=true;clearInterval(timer);window.removeEventListener('resize',resize);window.removeEventListener('liveops-action-completed',actionComplete);map.dragging.disable();map.off();map.remove();delete window.__bkbLiveOpsMapDestroy;container.dataset.leafletReady='0';document.removeEventListener('livewire:navigating',destroy)};window.__bkbLiveOpsMapDestroy=destroy;document.addEventListener('livewire:navigating',destroy,{once:true})};document.addEventListener('DOMContentLoaded',init,{once:true});document.addEventListener('livewire:navigated',init)})();
</script>
@endpush
</x-filament-panels::page>

...
map.on('contextmenu',e=>{ctx=e.latlng;const lat=document.getElementById('mx-context-lat'),lng=document.getElementById('mx-context-lng');lat.value=ctx.lat;lng.value=ctx.lng;lat.dispatchEvent(new Event('input',{bubbles:true}));lng.dispatchEvent(new Event('input',{bubbles:true}));const menu=document.getElementById('mx-context'),p=map.latLngToContainerPoint(ctx),stage=document.querySelector('.mx-stage');menu.classList.add('open');menu.style.left=Math.max(8,Math.min(p.x,stage.clientWidth-menu.offsetWidth-8))+'px';menu.style.top=Math.max(8,Math.min(p.y,stage.clientHeight-menu.offsetHeight-8))+'px';menu.focus({preventScroll:true});document.getElementById('mx-coordinates').textContent=`${ctx.lat.toFixed(6)}, ${ctx.lng.toFixed(6)}`;document.getElementById('mx-external-map').href=`https://www.openstreetmap.org/?mlat=${ctx.lat}&mlon=${ctx.lng}#map=16/${ctx.lat}/${ctx.lng}`});map.on('click',()=>document.getElementById('mx-context').classList.remove('open'));document.addEventListener('keydown',e=>{if(e.key==='Escape')document.getElementById('mx-context').classList.remove('open')});
document.querySelectorAll('[data-action=zone]').forEach(b=>b.addEventListener('click',()=>{const type=b.dataset.zone;window.Livewire?.find(@json($this->getId()))?.set('zoneType',type);document.getElementById('mx-zone-editor').classList.add('open');document.getElementById('mx-context').classList.remove('open')}));document.querySelector('[data-action=copy]').addEventListener('click',()=>ctx&&navigator.clipboard.writeText(`${ctx.lat},${ctx.lng}`));document.querySelector('[data-action=dispatch-note]')?.addEventListener('click',()=>window.Livewire?.find(@json($this->getId()))?.call('addDispatchNoteAtLocation'));document.querySelector('[data-action=support]')?.addEventListener('click',()=>window.Livewire?.find(@json($this->getId()))?.call('createSupportAtLocation'));document.getElementById('mx-close-editor').addEventListener('click',()=>document.getElementById('mx-zone-editor').classList.remove('open'));document.getElementById('mx-refresh').addEventListener('click',refresh);refresh();const timer=setInterval(refresh,Math.max(10,d.refresh_seconds||12)*1000);const destroy=()=>{if(destroyed)return;destroyed=true;clearInterval(timer);window.removeEventListener('resize',resize);map.off();map.remove();container.dataset.leafletReady='0';document.removeEventListener('livewire:navigating',destroy)};document.addEventListener('livewire:navigating',destroy,{once:true})};document.addEventListener('DOMContentLoaded',init,{once:true});document.addEventListener('livewire:navigated',init)})();
</script>
@endpush
</x-filament-panels::page>

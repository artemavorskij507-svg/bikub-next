...
@php
    $activity = $selectedTicket
        ...
        : collect();
@endphp
<x-admin-os.module-shell class="sc" label="Support Center command workspace">
    ...
    <section class="sc-kpis" aria-label="Support metrics">
        @foreach([
            ...
            ['unassigned','Unassigned','info'], ['waiting_customer','Waiting customer','info'], ['waiting_worker','Waiting worker','cyan'],
    ...
    <div class="sc-grid">
        <section class="sc-panel sc-queue">
            ...
            <div class="sc-tabs" role="tablist" aria-label="Ticket queues">
                @foreach(['incoming'=>'Incoming','all'=>'All','mine'=>'Mine','urgent'=>'Urgent','unassigned'=>'Unassigned','waiting_customer'=>'Waiting customer','waiting_worker'=>'Waiting worker','resolved'=>'Resolved'] as $key=>$label)
            ...
            <div class="sc-filters" aria-label="Ticket categories">
                @foreach(['all'=>'All','order_issue'=>'Orders','payment_issue'=>'Payments','worker_issue'=>'Workers','document_issue'=>'Documents','system_issue'=>'Technical'] as $key=>$label)
                    <button type="button" wire:click="setCategoryFilter('{{ $key }}')" class="{{ $categoryFilter === $key ? 'is-active' : '' }}" aria-pressed="{{ $categoryFilter === $key ? 'true' : 'false' }}">{{ $label }}</button>
                @endforeach
            </div>
            ...
            <div class="sc-queue-list">
                @forelse($activeQueue as $ticket)
                    <x-admin-os.queue-card wire:click="selectTicket({{ $ticket->id }})" class="sc-row" :selected="$selectedTicket?->is($ticket)">
        ...
        <main class="sc-panel sc-conversation">
            @if($selectedTicket)
                ...
                <div class="sc-ticket-facts">
                    <div><span>Channel</span><strong>{{ str($selectedTicket->source)->replace('_',' ')->title() }}</strong></div>
                    <div><span>Visibility</span><strong>{{ str($selectedTicket->visibility)->replace('_',' ')->title() }}</strong></div>
                    <div><span>Messages</span><strong>{{ $selectedTicket->messages->count() }}</strong></div>
                    <div><span>Events</span><strong>{{ $selectedTicket->events->count() }}</strong></div>
                </div>
                <div class="sc-summary"><span>Issue summary</span><p>{{ $selectedTicket->summary ?: 'No summary provided. Review linked context and timeline before taking action.' }}</p></div>
                ...
                @can('admin.support.manage')
                    <section class="sc-decisions" aria-label="Ticket decision controls">
                        <label><span>Required decision note</span><input wire:model="decisionNote" type="text" placeholder="Reason required before escalation or resolution"></label>
                        <button type="button" wire:click="escalateSelected" class="is-warning" title="Escalate with the entered decision note" @disabled(in_array($selectedTicket->status, ['escalated','resolved','closed'], true))>Escalate</button>
                        @can('admin.support.resolve')
                            <button type="button" wire:click="resolveSelected" class="is-success" title="Resolve with the entered decision note" @disabled(in_array($selectedTicket->status, ['resolved','closed'], true))>Resolve</button>
                        @endcan
                        @error('decisionNote')<p>{{ $message }}</p>@enderror
                    </section>
                    <form class="sc-composer" wire:submit="sendMessage">
                        <div class="sc-composer-tabs" role="tablist" aria-label="Message visibility">
                            <button type="button" role="tab" aria-selected="{{ $composerMode==='internal'?'true':'false' }}" wire:click="setComposerMode('internal')" class="{{ $composerMode==='internal'?'is-active':'' }}">Internal note</button>
                            <button type="button" role="tab" aria-selected="{{ $composerMode==='customer'?'true':'false' }}" title="{{ $selectedTicket->customer_id ? 'Write a customer-visible reply' : 'No linked customer account' }}" wire:click="setComposerMode('customer')" class="{{ $composerMode==='customer'?'is-active':'' }}" @disabled(!$selectedTicket->customer_id)>Customer reply</button>
                            <button type="button" role="tab" aria-selected="{{ $composerMode==='worker'?'true':'false' }}" title="{{ $selectedTicket->worker_profile_id ? 'Write a worker-visible reply' : 'No linked worker profile' }}" wire:click="setComposerMode('worker')" class="{{ $composerMode==='worker'?'is-active':'' }}" @disabled(!$selectedTicket->worker_profile_id)>Worker reply</button>
                        </div>
                ...
                @else
                    <p class="sc-readonly">You have read-only support access. Open the full ticket for available permitted actions.</p>
                @endcan
            @else

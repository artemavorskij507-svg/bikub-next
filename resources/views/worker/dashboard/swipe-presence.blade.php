<section class="wv2-bottom wv2-glass {{ $activeOrder ? 'is-assigned' : ($online ? 'is-waiting' : 'is-offline') }}" aria-label="Worker presence control">
  @if($activeOrder)
    <p class="wv2-kicker">Assigned order</p>
    <h3>{{ $activeOrder->order_number }}</h3>
    <p class="wv2-swipe-status">{{ $activeOrder->scenario?->title ?? $activeOrder->service_scenario_key ?? 'Assigned service' }}</p>
    @if($pickup)
      <p class="wv2-swipe-status">Pickup: {{ $pickup }}</p>
    @endif
    <div class="wv2-sheet-actions">
    ...
    <p id="wv2-gps-status" class="wv2-swipe-status">{{ $gpsState }} · one real GPS ping only; no background tracking.</p>
  @else

...
@php
    ...
    $serviceLanes = [
        ['icon'=>'🛒','label'=>'Grocery','key'=>'Stores','enabled'=>(bool)($wp?->can_deliver),'note'=>(($wp?->can_deliver) ? 'delivery capable' : 'not configured')],
        ['icon'=>'🍔','label'=>'Ready food','key'=>'Food','enabled'=>(bool)($wp?->can_deliver),'note'=>(($wp?->can_deliver) ? 'delivery capable' : 'not configured')],
        ['icon'=>'🏗','label'=>'Bulky/materials','key'=>'Bulky','enabled'=>(bool)($wp?->can_move),'note'=>(($wp?->can_move) ? 'configured' : 'not configured yet')],
        ['icon'=>'🤝','label'=>'Errands','key'=>'Assistant','enabled'=>(bool)($wp?->can_run_errands),'note'=>(($wp?->can_run_errands) ? 'configured' : 'not configured yet')],
        ['icon'=>'📍','label'=>'Narvik zone','key'=>'Pilot','enabled'=>$profileApproved,'note'=>$profileApproved ? 'approved worker' : 'approval needed'],
    ];
    $mapPayload = [
